...
::Variables utilisateur, à régler selon vos besoins
set project_author=Shadow256
::set git_project_name=switch_AIO_LS_pack
::set git_project_local_path=.\switch_aio_ls_pack
::set git_project_remote_url=https://github.com/shadow2560/switch_AIO_LS_pack.git

cls
set project_name=%~n0
set script_base_path=%~dp0
set this_script_full_path=%~0
set language_custom=0
:define_language_path
set language_path=
IF NOT EXIST "%project_name%.bat.lng" (
	call :config_lng
)
set /p language_path=<"%project_name%.bat.lng"
...
:define_theme
set theme=
IF NOT EXIST "%project_name%.bat.theme" (
	call :config_theme
)
set /p theme=<"%project_name%.bat.theme"
color %theme%
...
chcp 65001 >nul
IF /i NOT "%debug_mode%"=="on" (
	call tools\Storage\menu.bat
) else (
	call tools\Storage\menu.bat 2>&1 | tools\gnuwin32\bin\tee.exe "%project_name%.log" >con
)
goto:end_script
...
mkdir "templogs"
IF EXIST "%project_name%.bat.lng\*.*" (
	rmdir /s /q "%project_name%.bat.lng"
)
:set_temp_language
cls
...
copy nul "%project_name%.bat.lng" >nul
echo %temp_language_path%>>"%project_name%.bat.lng"
rmdir /s /q "templogs" 2>nul
cls
exit /b
...
mkdir "templogs"
IF EXIST "%project_name%.bat.theme\*.*" (
	rmdir /s /q "%project_name%.bat.theme"
)
:set_temp_theme
cls
...
set /p temp_theme=<templogs\tempvar.txt
copy nul "%project_name%.bat.theme" >nul
echo %temp_theme%>>"%project_name%.bat.theme"
rmdir /s /q "templogs" 2>nul
cls
exit /b
